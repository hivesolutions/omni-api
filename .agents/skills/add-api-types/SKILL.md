...
4. **Write the `Delta`** mirroring the entity chain at the same level (`Sale(Operation)` -> `SaleDelta(OperationDelta)`). Exclude `secure=True` and `immutable` fields - colony's apply silently skips secure attributes, so typing them would promise something the server ignores. Reference relations with `BaseReference` (`{object_id}`); polymorphic relations take a `_class` key, declared once on `BaseDelta` as `NotRequired[str]` (colony's apply accepts the reserved `_class` on every entity map, mirroring `Base._class`) - functionally required for polymorphic creation (`{"_class": "CashPayment"}`) but not type-required, same as other validation-level requirements. `_parameters` stays per-delta since its shape is flow-specific (`SaleCustomerParameters`).
...
- **Declare every field at the stub class matching the Omni class that defines it** - grep the model chain before placing a field. `workflow_state` belongs to `WorkflowOperation` (not `Transfer`), `username`/`email` belong to `BaseUser` (not `SystemUser`), `Customer` is `(Contactable, Identifiable)`. Getting the level wrong is not fixable downstream: a TypedDict subclass cannot re-declare an inherited key, and a missing base class silently drops all its keys from the type. When a base-declared field has per-subclass value sets (`workflow_state`), the base declares it as a generic TypedDict parameter (`class WorkflowOperation(Operation, Generic[_W])` with `workflow_state: _W` and `_W = TypeVar("_W", bound=int, default=int)`) and each subclass binds its own enumeration (`class Transfer(WorkflowOperation[TransferStateT])`).
